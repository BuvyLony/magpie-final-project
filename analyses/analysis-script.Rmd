---
title: "Magpie Final Project"
author: "Bensien Marie, Martirosov Iolanta, Matheis Clara, Ohnesorge Alina"
date: "25 7 2021"
...
#Read in data
all_data <- read.csv("data/Exp_data.csv")

#Create readable labels for plotting.
all_data <- mutate(all_data, condition = ifelse(ingroupNorm==1 & bothShown==0, "Ingroup norm favoured \n leaving robber alone \n Only ingroup norm shown",
...
all_data$responseLabels <- ordered(all_data$response, labels = c("1 = Definitely call the police", "2", "3", "4", "5", "6 = Definitely leave the robber alone"))
...
# #Demographics---------------
# demographics_summary <- group_by(all_data,expNum) %>%
...
ordinal_1 <- clm(as.factor(response)~ingroupNorm*bothShown, data=useable_data)
...
#--Fit models
fit_SCT_1 <- stan(file = "analysis/stan_models/SCT.stan", data=stan_useable_data, iter=10000, chains=4, seed = 123, control=list(adapt_delta = 0.99))
fit_herding_1 <- stan(file = "analysis/stan_models/herding.stan", data=stan_useable_data, iter=10000, chains=4, control=list(adapt_delta = 0.99))
