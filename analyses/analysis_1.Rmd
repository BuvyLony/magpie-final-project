...
analyse_data <- all_data[-c(2,4,5,6,7,8,9,10,11,21,22,23,25)]
#exclude previous data -> comment out later
analyse_data <- analyse_data %>% 
  filter(submission_id >= 2628 & submission_id<= 2632)
...
analyse_data <- analyse_data %>%
  mutate(ingroup_norm_new = case_when(grepl("chose to do nothing and leave the robber alone.$",ingroup_norm) ~ 1,
                           grepl("chose to call the police and report the robber.$",ingroup_norm) ~ 0))
...
#analyse_data <- as.numeric(analyse_data)
...
analyse_data <- analyse_data %>%
  select(submission_id,moral_dilemma, ingroup_norm_new, both_norms_shown, identification_pro, identification_anti) #%>% #select relevant columns
...
#stan_data_all <- as.list(c(filter(analyse_data), N=sum(analyse_data)))
