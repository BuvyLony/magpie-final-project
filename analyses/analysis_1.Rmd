...
# new labels for better understanding: TODO maybe keep for plotting purposes
...
for(i in 1:nrow(analyse_data)){
  if(analyse_data[i,"issue"] == "Gun control"){
...
# fit regression model and look at parameters TODO: read up how to ?
ordinal_1 <- clm(as.factor(moral_dilemma)~ ingroup_norm_new*both_norms_shown, data=analyse_data)
ordinal_1
# produce plot for answer dist TODO maybe delete
...
data_plot<- likert(analyse_data[,"Response to moral dilemma from 'definitely report' to 'definitely leave alone'",drop=FALSE],grouping = analyse_data$both_norms_shown)


plot1 <-plot(data_plot)  + theme(axis.text=element_text(color="black",size=9),legend.position="bottom")
plot1
ggsave("plot1.png", height = 30, width = 10)
```
