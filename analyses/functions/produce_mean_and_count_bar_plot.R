produce_mean_and_count_bar_plot <- function(df, bar_width_means=0.5, bar_width_response=0.4,   arrow_length = 0.4){
  ...
  response_counts = group_by(df, ingroup_norm_new, both_norms_shown, response)%>%
  ...
  props <- group_by(df, ingroup_norm_new, both_norms_shown, response) %>%
  ...
  means$both_norms_shown = factor(means$both_norms_shown, labels=c("Only ingroup norm shown", "Both norms shown"))
  ...
  #   mutate(arrowx = ingroup_norm_new + bar_width_means/1.3 + theory/10,
  #          arrowxend = arrowx,
  #          arrowy = meanResponse,
  #          arrowyend = ifelse(ingroup_norm_new==0 & theory==0, arrowy - arrow_length ,
  #                             ifelse(ingroup_norm_new==0 & theory==1, arrowy + arrow_length ,
  #                                    ifelse(ingroup_norm_new==1 & theory==0,arrowy + arrow_length , arrowy - arrow_length))),
  #          arrowAlpha = ifelse(both_norms_shown=="Both norms shown", 0, 1))
  
  
  library(scales) #Needed to give rescale_none function to oob. 
  return_plot <- ggplot()+
    geom_col(data=means, aes(x=ingroup_norm_new, y=meanResponse, fill=factor(ingroup_norm_new)), width=bar_width_means, alpha=0.5) +
    geom_rect(data=props, aes(xmin=xmin, ymin=ymin, xmax=xmax, ymax=ymax, group=ingroup_norm_new), fill = "#9c9d9e", alpha = 0.8) +
